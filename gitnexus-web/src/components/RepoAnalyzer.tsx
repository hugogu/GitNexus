...
export const RepoAnalyzer = ({
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const gitlabConfig = settings.gitlab;
      const request: { url?: string; path?: string; token?: string } =
        mode === 'github'
          ? { url: githubUrl.trim() }
          : mode === 'gitlab'
            ? { url: gitlabUrl.trim() }
      ...
      // Add GitLab token if configured and enabled
      if (mode === 'gitlab' && gitlabConfig?.enabled && gitlabConfig?.token) {
        request.token = gitlabConfig.token;
      }
